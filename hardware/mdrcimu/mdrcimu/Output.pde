...
//		Serial.print(press>>2);       // Convert into Pa       
...
#elif MDRC_I2C_OUT
  //Adjust IMU heading based on last compass delta, and normalize 0 <-> 360
  float fyaw = ToDeg(yaw);
  if (fyaw < 0) fyaw += 360;
  
  unsigned int tempuint;
  
  float tYaw = fyaw + dYaw;
  if (tYaw > 360.0) tYaw -= 360;
  else if (tYaw < 0.0) tYaw += 360;
  
  Wire.beginTransmission(ROS_ADDR);
  
  int a = 4;
  int tempint;
  
  //Start of message *************************************
  tempint = ToDeg(roll) * 100;
  ...
  i2c_buf[a++] = (tempint >> 8) & 0xFF;
  i2c_buf[a++] = (tempint) & 0xFF;
  
  tempint = ToDeg(pitch) * 100;
  ...
  Serial.print(",::");
  Serial.print((int)(fyaw * 100), DEC);
  Serial.print(",");
  Serial.println((int)(dYaw * 100), DEC);
  #endif
  
  i2c_buf[a++] = (tempuint >> 8) & 0xFF;
  i2c_buf[a++] = (tempuint) & 0xFF;
  //End of message **********************************************
